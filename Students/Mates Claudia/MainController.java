...
public class MainController {
	...
	private void initializeButtonListeners() {
		...
		/*logindirectorView.addVizualizareButtonActionListener(e ->{
		   final String username0 = loginView.getUsername();
			final String password0 = loginView.getPassword();
			ResultSet rs0;
			String salariu="0";
			int c0 = 0;
			...
			try {
...

				{
					...
					rs0 = dConnection
							.executeStatement("select nume,prenume,salariu from Agent_inchirieri where nume='" + username0  + "' and prenume='"+password0+"';");
					while (rs.next()) {
						if ((username.equals(rs.getString("nume"))) && (password.equals(rs.getString("prenume")))) {
							c = 1;
